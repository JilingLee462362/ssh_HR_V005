...
<body>
<!-- 将整个页面作为一个表单提交 -->
<form action="er/resumeSearch_hr.action" method="post" id="mainform">
	<div id="container">
						...
						<table style="margin-left:0px;width:100%;border=" 1" class="table">
								...
								<tr>
									<th>
										<div class="ui-table-th">
											<span class="ui-table-thTitle">&nbsp;&nbsp;&nbsp;&nbsp;姓名</span>
											<span class="ui-table-drag"></span>
										</div>
									</th>
									<th>
										<div class="ui-table-th">
											<span class="ui-table-thTitle">身份证</span>
											<span class="ui-table-drag"></span>
										</div>
									</th>
									...
									<th>
										<div class="ui-table-th">
											<span class="ui-table-thTitle">应聘职位</span>
											<span class="ui-table-drag"></span>
										</div>
									</th>
							...
							<tbody>
								...
								<c:forEach items="${requestScope.listEngageResume}" var="ler">
									<tr>
										<td>&nbsp;&nbsp;&nbsp;&nbsp;${ler.humanName}</td>
										<td>${ler.humanIdcard}</td>
										<td>${ler.humanMajorName}</td>
										<c:if test="${ler.checkStatus==0}"><td>待审核</td></c:if>
										<c:if test="${ler.checkStatus==1}"><td>审核通过</td></c:if>
										<c:if test="${ler.checkStatus==2}"><td>未通过</td></c:if>
										<td><a href="er/resumeinfo.action?resId=${ler.resId}">查看详情</a></td>
									</tr>
								</c:forEach>
...
<script type="text/javascript">
	$(document).ready(function(){
	    ...
	    if($("#checkStatus").val() != null && $("#checkStatus").val() != ""){
	    	if ($("#checkStatus").val() == 1 ){
	    		$("#checkStatusSelect").get(0).selectedIndex=2;
	    	}else if ($("#checkStatus").val() == 2){
	    		$("#checkStatusSelect").get(0).selectedIndex=3;
	    	}else {
	    		$("#checkStatusSelect").get(0).selectedIndex=1;
	    	}
	    	
	    }else{
	    	//对应select的option设置为全部
	    	$("#checkStatusSelect").get(0).selectedIndex=0;
	    	$("#checkStatus").attr("value", $("#checkStatusSelect option:selected").val());
	    }
	...
	$(document).ready(function() {
  		...
  		$("#checkStatusSelect").change(function(){
  			var choose = $("#checkStatusSelect  option:selected").text();
  			if (choose=="待审核") {
  				$("#checkStatus").attr("value", 0);
  			} else if (choose=="审核通过") {
  				$("#checkStatus").attr("value", 1);
  			} else if (choose=="未通过") {
  				$("#checkStatus").attr("value", 2);
  			} else {
  				$("#checkStatus").attr("value", "");
  			}
  		});
